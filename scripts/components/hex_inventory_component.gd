...
var _grid_slots: Dictionary = {} # Vector2i -> HexItemData
...
func get_all_placed_items() -> Array[HexItemData]:
...
func get_com_offset_2d() -> Vector2:
	...
	for item: HexItemData in _placed_items:
		...
		var footprint: Array[Vector2i] = item.get_rotated_footprint(item.rotation_step)
		for offset: Vector2i in footprint:
