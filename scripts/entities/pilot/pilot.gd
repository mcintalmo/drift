...
func mount_into_sled(sled: CharacterBody3D) -> void:
	...
	if sled and "is_occupied" in sled:
...
func dismount_from_sled() -> void:
	...
	var sled_base_pos: Vector3 = current_sled.global_position if current_sled.is_inside_tree() else current_sled.position
	var sled_basis_x: Vector3 = current_sled.global_transform.basis.x if current_sled.is_inside_tree() else Vector3.RIGHT
	var dismount_pos: Vector3 = sled_base_pos + Vector3(0, 1.2, 0) + (sled_basis_x * 1.5)
	var sled_vel: Vector3 = current_sled.velocity
	
	is_mounted_in_sled = false
	var prev_sled: CharacterBody3D = current_sled
	if prev_sled and "is_occupied" in prev_sled:
