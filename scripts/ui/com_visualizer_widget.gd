...
@export var max_offset_display_m: float = 0.20 # 20cm full gauge radius

@export_group("Thresholds")
@export var safe_threshold_m: float = 0.04
@export var warning_threshold_m: float = 0.08
...
func _ready() -> void:
	...
	if GlobalEvents.instance:
		GlobalEvents.instance.inventory_updated.connect(func(_container_id: StringName, mass: float, com: Vector2) -> void:
			update_com_data(mass, com)
		)

func update_com_data(mass: float, com: Vector2) -> void:
	current_mass_kg = mass
	_target_com_offset = com
	queue_redraw()

func _process(delta: float) -> void:
	if current_com_offset.distance_to(_target_com_offset) > 0.0005:
		current_com_offset = current_com_offset.lerp(_target_com_offset, 12.0 * delta)
		queue_redraw()
...
func _draw() -> void:
	...
	# Center anchor cross
	draw_line(center + Vector2(-6, 0), center + Vector2(6, 0), Color(1, 1, 1, 0.4), 1.0)
	draw_line(center + Vector2(0, -6), center + Vector2(0, 6), Color(1, 1, 1, 0.4), 1.0)
	...
	if offset_dist >= warning_threshold_m:
		dot_color = Color(1.0, 0.25, 0.2, 1.0)
	...
	draw_circle(crosshair_pos, 2.0, Color.WHITE)
	
	# 4. Draw Lateral Bias Balance Bar at bottom
	var bar_y: float = size.y - 26.0
	...
	draw_line(Vector2(bar_center_x, bar_y - 5), Vector2(bar_center_x, bar_y + 5), Color(1, 1, 1, 0.7), 2.0)
	...
	draw_circle(Vector2(pip_x, bar_y), 5.0, dot_color)
	
	# Readout text
	var stats_text: String = "%d kg | Lat: %+.2fm" % [int(current_mass_kg), current_com_offset.x]
	draw_string(ThemeDB.fallback_font, Vector2(10, size.y - 6), stats_text, HORIZONTAL_ALIGNMENT_CENTER, int(size.x - 20), 10, Color(0.85, 0.9, 0.95, 0.9))
