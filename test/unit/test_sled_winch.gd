...
func run_tests() -> Array[Dictionary]:
	...
	results.append(_test_winch_reeling_shortens_cable_length())
...
func _test_spring_force_proportional_to_stretch() -> Dictionary:
	...
	var passed: bool = force.length() > 450.0 and force.z > 0.0
	...
	return {
		...
		"passed": passed,
		"message": "5m stretch produced %f N (expected > 450.0 N)" % force.length()
	}

func _test_winch_reeling_shortens_cable_length() -> Dictionary:
	var winch: SledWinchComponent = SledWinchComponent.new()
	winch.winch_data = WinchData.new()
	winch.winch_data.reel_in_speed_ms = 20.0
	
	var anchor: GrappleAnchorComponent = GrappleAnchorComponent.new()
	anchor.position = Vector3(0.0, 0.0, 20.0) # 20m away
	winch.attach_to_anchor(anchor)
	
	var initial_rest_len: float = winch._rest_cable_length_m
	winch.set_reeling(true)
	winch.compute_tether_force(0.5, Vector3.ZERO)
	var final_rest_len: float = winch._rest_cable_length_m
	
	var passed: bool = final_rest_len < initial_rest_len and final_rest_len >= 2.5
	
	winch.free()
	anchor.free()
	return {
		"name": "test_winch_reeling_shortens_cable_length",
		"passed": passed,
		"message": "Active reeling shortened rest length: %.1fm -> %.1fm (passed: %s)" % [initial_rest_len, final_rest_len, str(passed)]
	}
...
func _test_sled_winch_moving_train_tether() -> Dictionary:
	...
	winch.winch_data.tensile_limit_force = 5500.0
